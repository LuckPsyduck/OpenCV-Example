...
int main(int argc,char **argv)
{
	...
	namedWindow("【原始图】",0); 
	namedWindow("【效果图】顶帽运算",0); 
	namedWindow("【效果图】底帽运算",0); 

	imshow("【原始图】", image); 
	
	Mat element = getStructuringElement(MORPH_RECT, Size(15, 15)); //定义核
	
	morphologyEx(image,image_TopHat, MORPH_TOPHAT, element);//进行形态学操作

	morphologyEx(image,image_BlockHat, MORPH_BLACKHAT, element);

	imshow("【效果图】顶帽运算", image_TopHat); 
	imshow("【效果图】底帽运算", image_BlockHat); 
